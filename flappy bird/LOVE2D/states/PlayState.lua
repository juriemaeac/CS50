...
function PlayState:update(dt)
    ...
    if self.timer > spawner then
		...
		-- reset spawner for randomization
		spawner = 0
		
		-- reset gap height for randomization
		GAP_HEIGHT = 0
    end
	...
	if love.keyboard.wasPressed('p') and scrolling == true then
		...
		sounds['music']:pause()
		sounds['pause']:play()
	elseif love.keyboard.wasPressed('return') and scrolling == false then
		scrolling = true
		sounds['music']:play()
	end
end
...
function PlayState:render()
    ...
    if scrolling == true then
	    ...
	    love.graphics.print('To pause the game, press P!', 8, VIRTUAL_HEIGHT - 50)
	end
	...
	if scrolling == false then
		...
		love.graphics.setFont(smallFont)
		love.graphics.printf('Press enter to continue playing.', 0, 100, VIRTUAL_WIDTH, 'center')
